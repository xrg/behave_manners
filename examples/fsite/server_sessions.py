...
logger = logging.getLogger('server_sessions')
...
class CachingSessionManager(SessionManager):
    ...
    def _normalize(self):
        logger.info("Session cache size: %s", len(self._cache))
...
class FileBackedSessionManager(SessionManager):
    ...
    def remove(self, sid):
        logger.info('Removing session: %s', sid)
        fname = os.path.join(self.path, sid)
    ...
    def get(self, sid, digest):
        ...
        logger.info("Looking for session: %s, %s", sid, digest)
        ...
        if os.path.exists(fname):
            try:
                ...
                logger.warning("Error loading session file", exc_info=True)
        ...
        if hmac_digest != digest:
            logger.warning("Invalid HMAC for session")
            return self.new_session()
    ...
    def put(self, session):
        'Store a managed session'
        logger.info("Storing session: %s", session.sid)
